...
const Card = ({ href, title, text }: {href: string; title: string; text?: string}) => {
  return (
    <div className="mr-4 mb-8">
      <div className="opacity-60 max-w-sm max-h-xs p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <Link href={href}>
          <h5 className="opacity-100 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{title}</h5>
        </Link>
        <p className="pt-2 pb-2 font-normal text-gray-700 dark:text-gray-400">{text}</p>
